...
export const Route = createFileRoute("/_authenticated/panel")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Revisa el estado, las visitas y los contactos de tu emprendimiento en La Vitrina.",
      },
...
function PanelPage() {
  ...
  const totals = (mine.data ?? []).reduce(
    (acc, e) => ({ views: acc.views + e.views, contacts: acc.contacts + e.contacts }),
    { views: 0, contacts: 0 },
  );
    ...
    <section className="container-page py-12">
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild>
            <Link to="/sumate">
              <Plus className="h-4 w-4" /> Nuevo emprendimiento
            </Link>
          </Button>
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        <Stat label="Emprendimientos" value={mine.data?.length ?? 0} />
      ...
      <div className="mt-10 space-y-4">
        {mine.isLoading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border p-12 text-center">
            <p className="font-display text-xl">Todavía no publicas tu emprendimiento</p>
            ...
            >
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="font-display text-xl font-semibold">{e.business_name}</h2>
                  <Badge variant={e.status === "aprobado" ? "default" : "secondary"}>
                    {STATUS_LABEL[e.status]}
                  </Badge>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{e.short_description}</p>
                <p className="mt-2 flex gap-4 text-xs text-muted-foreground">
                  <span className="inline-flex items-center gap-1">
                    <Eye className="h-3.5 w-3.5" /> {e.views} visitas
                  </span>
                  <span className="inline-flex items-center gap-1">
                    <MessageCircle className="h-3.5 w-3.5" /> {e.contacts} contactos
                  </span>
                </p>
              </div>
              {e.status === "aprobado" ? (
